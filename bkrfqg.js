...
// ════════════════════════════════════════════════════════════════════
// ====================================================================
// ANWESENHEITSLISTE - ein Blatt je Kurstag
// ====================================================================
function bkrfqgDruckenAnwesenheit() {
  const kp = bkrfqgState.kursplaene.find(x => x.id === bkrfqgKPSelected);
  const kt = bkrfqgKPKurstage;
  ...
  // Teilnehmer: Personendaten haengen an schulung_participants.
  // Liegen keine vor, wird mit Leerzeilen gedruckt.
  const tnListe = (bkrfqgKPTeilnehmer || []).map(function(t){
    const p = (typeof bgqPerson === 'function') ? bgqPerson(t) : (t.schulung_participants || null);
    return {
      name: p ? ((p.last_name||'') + ', ' + (p.first_name||'')).replace(/^, |, $/,'')
              : ((t.nachname||'') + ', ' + (t.vorname||'')).replace(/^, |, $/,''),
      geb: (p && p.birth) ? bfmtD(p.birth) : ''
    };
  }).filter(function(x){ return x.name; })
    .sort(function(a,b){ return a.name.localeCompare(b.name, 'de'); });

  if (!tnListe.length) toast('Keine Teilnehmer zugeordnet - es werden Leerzeilen gedruckt');

  const bandTitelAus = function(g){
    const m = (g||'').match(/^Band [^:]+:\s*(.*)/);
    return m ? m[1].trim() : (g || '');
  };

  // Kurstage nach Datum buendeln - ein Datum ergibt ein Blatt.
  const tage = {};
  kt.forEach(function(k){ if(!tage[k.datum]) tage[k.datum] = []; tage[k.datum].push(k); });
  const daten = Object.keys(tage).sort();

  const blaetter = daten.map(function(datum, idx){
    const sessions = tage[datum].slice().sort(function(a,b){
    ...
    const tagH = Math.round(sessions.reduce(function(s,k){ return s + (k.stunden||0); },0)*10)/10;

    const beg = (sessions[0] && sessions[0].beginn ? sessions[0].beginn.slice(0,5) : '08:00');
    const letzte = sessions[sessions.length-1];
    const end = (letzte && letzte.ende ? letzte.ende.slice(0,5) : '15:45');

    const dozenten = [];
    sessions.forEach(function(k){
      const m = k.mitarbeiter;
      if (m) { const n = (m.vorname||'') + ' ' + (m.nachname||'');
               if (dozenten.indexOf(n) < 0) dozenten.push(n); }
    });
    const themen = sessions.map(function(k){ return bandTitelAus(k.gegenstand); })
                           .filter(function(v,i,a){ return v && a.indexOf(v) === i; });

    const zeilen = [];
    const anzahl = Math.max(tnListe.length + 3, 12);
    ...
    '<div class="blatt">' +
      '<div class="kopf">' +
        ...
        '<div>' +
          '<div class="eyebrow">Anwesenheitsliste \u00b7 Berufskraftfahrer-Grundqualifikation</div>' +
          '<div class="titel">' + bEsc(bKursTypLabel(kp.kurstyp)) + '</div>' +
          '<div class="sub">' + bEsc((kp.bkrfqg_standorte && kp.bkrfqg_standorte.name) || 'Fahrschulteam Lingen') +
            ' \u00b7 ' + bEsc(kp.titel || '') + '</div>' +
        '</div>' +
        '<div class="tagnr">Kurstag<br><strong>' + (idx+1) + ' / ' + daten.length + '</strong></div>' +
      '</div>' +
      '<div class="infobox">' +
        '<div><span>Datum</span><strong>' + WT[d.getDay()] + ', ' + datStr + '</strong></div>' +
        '<div><span>Uhrzeit</span><strong>' + beg + ' \u2013 ' + end + ' Uhr\u00a0\u00b7\u00a0' + tagH + ' Std.</strong></div>' +
        '<div><span>Dozent</span><strong>' + (dozenten.length ? bEsc(dozenten.join(', ')) : '\u2013') + '</strong></div>' +
      '</div>' +
      (themen.length ? '<div class="themen"><span>Themen</span>' + bEsc(themen.join(' \u00b7 ')) + '</div>' : '') +
      '<table>' +
        '<thead><tr>' +
          '<th class="nr">Lfd. Nr</th>' +
          '<th>Name, Vorname</th>' +
          '<th class="geb">Geb.-Datum</th>' +
          '<th class="sig">Unterschrift</th>' +
        '</tr></thead>' +
        '<tbody>' + zeilen.join('') + '</tbody>' +
      '</table>' +
      '<div class="unten">' +
        '<div class="linie">Ort, Datum</div>' +
        '<div class="linie">Unterschrift Dozent / Ausbilder</div>' +
      '</div>' +
      '<div class="fuss">' +
        '<span>Fahrschulteam Lingen GmbH \u00b7 Rheiner Str. 158 \u00b7 49809 Lingen (Ems) \u00b7 AZAV-Nr. 0333-10660-AZAV-T</span>' +
        '<span>Blatt ' + (idx+1) + ' von ' + daten.length + '</span>' +
      '</div>' +
    '</div>';
  }).join('');

  const html = '<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8">' +
  '<title>Anwesenheitsliste \u2013 ' + bEsc(kp.titel || '') + '</title><style>' +
  '*{margin:0;padding:0;box-sizing:border-box}' +
  'body{font-family:Arial,Helvetica,sans-serif;font-size:10pt;color:#3F4B57;background:#fff}' +
  '@page{size:A4;margin:12mm 10mm 10mm}' +
  '@media screen{body{max-width:210mm;margin:0 auto;padding:14mm 12mm}' +
    '.blatt{margin-bottom:24px;border-bottom:2px dashed #ddd;padding-bottom:24px}}' +
  '@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
    '.blatt{page-break-after:always}.blatt:last-child{page-break-after:auto}}' +
  '.kopf{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;' +
    'border-bottom:3px solid #C0001A;padding-bottom:8px;margin-bottom:12px}' +
  '.logo{height:44px;object-fit:contain;margin-right:12px;float:left}' +
  '.eyebrow{font-size:8pt;color:#C0001A;font-weight:700;letter-spacing:.05em;text-transform:uppercase}' +
  '.titel{font-size:14pt;font-weight:800;margin:2px 0;color:#1a1a1a}' +
  '.sub{font-size:8.5pt;color:#555}' +
  '.tagnr{font-size:8pt;color:#555;text-align:right;white-space:nowrap}' +
  '.tagnr strong{font-size:13pt;color:#C0001A}' +
  '.infobox{display:flex;gap:18px;background:#f9f9f9;border:1px solid #e5e5e5;border-radius:4px;' +
    'padding:7px 12px;margin-bottom:8px;font-size:9pt}' +
  '.infobox div{display:flex;flex-direction:column}' +
  '.infobox span{font-size:7.5pt;color:#888;text-transform:uppercase;letter-spacing:.04em}' +
  '.themen{font-size:8.5pt;color:#555;margin-bottom:10px;padding-left:2px}' +
  '.themen span{font-size:7.5pt;color:#888;text-transform:uppercase;letter-spacing:.04em;margin-right:6px}' +
  'table{width:100%;border-collapse:collapse}' +
  'th{background:#C0001A;color:#fff;font-size:8.5pt;font-weight:700;text-align:left;padding:5px 7px}' +
  'td{border:1px solid #d9d9d9;padding:0 7px;height:9mm;font-size:9.5pt;vertical-align:middle}' +
  'th.nr,td.nr{width:16mm;text-align:center;color:#888}' +
  'th.geb,td.geb{width:28mm}' +
  'th.sig,td.sig{width:72mm}' +
  'tbody tr:nth-child(even) td{background:#fafafa}' +
  '.unten{display:flex;gap:40px;margin-top:14px}' +
  '.linie{flex:1;border-top:1px solid #999;padding-top:3px;font-size:8pt;color:#777}' +
  '.fuss{display:flex;justify-content:space-between;font-size:7.5pt;color:#aaa;' +
    'border-top:1px solid #e5e5e5;margin-top:10px;padding-top:4px}' +
  '</style></head><body>' + blaetter +
