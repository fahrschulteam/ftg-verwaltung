    ...
    <div class="modal" style="width:760px">
      ...
      <div class="modal-body">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
          ...
          <div>
            <div class="fsec">??? Zuständige Behörde</div>
...
function bkrfqgRaeume(el) {
  ...
  el.innerHTML = bKopf('?? Unterrichtsräume', '§ 9 Abs. 3 BKrFQG – nur genehmigte Räume nutzbar',
    ...
    + `<div class="card" style="background:#fffbeb;border-color:#fde68a;padding:10px 14px;font-size:12px;margin-bottom:16px;color:#92400e">
        ?? Präsenzunterricht darf nur in den im Anerkennungsbescheid aufgeführten Räumen stattfinden.
      </div>`
...
async function bkrfqgKPSpeichern() {
  ...
  try {
    ...
    if (kiAktiv && kp && startVal) {
      ...
      if (_fehlende.length) {
        toast(`?? Kursplan unvollständig: Bänder ${_fehlende.map(b=>b.nr).join(', ')} fehlen. Bitte erneut generieren.`, 'warn', 9000);
      }
      ...
      if (konflikte.length) {
        toast(`?? ${konflikte.length} Dozenten-Konflikt(e) – bitte manuell prüfen`, 'warn', 8000);
  ...
  if (isKombi) {
    ...
    el.innerHTML += `
      <div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap">
        ...
        <span class="card" style="padding:6px 12px;font-size:12px;flex:0">S je Qualifikation: Güter <strong>${Math.round((gemH+gH)*10)/10}h</strong> / Person <strong>${Math.round((gemH+pH)*10)/10}h</strong> <span style="color:var(--grau);font-size:10px">· Soll ${sollGem+sollG}h</span></span>
      </div>`;
  }
...
async function bkrfqgKursmeldung(el) {
  ...
  try {
    ...
    el.innerHTML = bKopf('?? Kursmeldung','§ 11 Abs. 4 BKrFQG – Meldung 5 Werktage vor Unterricht',
      ...
      + `<div class="card" style="background:#fffbeb;border-color:#fde68a;padding:10px 14px;font-size:12px;margin-bottom:16px;color:#92400e">
          ?? Jede Schulung muss der Behörde spätestens 5 Werktage vorher gemeldet werden. Ausfall bis 1 Werktag vorher.
        </div>
        <div class="card" style="padding:14px 16px">
